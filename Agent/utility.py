...
def update():
    if not variables.updates:
        return
    else:
        return variables.updates[-1]


async def receive(uri):
    async with websockets.connect(uri) as websocket:
        inputs = await websocket.recv()
        return inputs
...
async def text_analysis(structure, websocket:WebSocket):
    missing_fields = {}
    raw_text=''
    with open(variables.project_file, 'r') as f:
        for i in f.readlines():
...
@app.websocket("/ws")
async def response_analysis(missing_fields, websocket:WebSocket):
    ...
    if len(missing_fields)>0:
        for i in fields:
            for j in (fields[i])[:]:
                ...
                5. End Requirement: If the input provides sufficient information to indicate task completeness or includes a request for further actions (e.g., generating content based on the provided information), respond with 'end requirement'.
...
#class containing functionalities to generate the charter
